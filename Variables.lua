...
function Gnosis:StartupVariables()
	...
	local autoShot = C_Spell.GetSpellInfo(75);
	self.strAutoShot, self.iconAutoShot = autoShot.name, autoShot.icon;
	local autoAttack = C_Spell.GetSpellInfo(6603);
	self.strAutoAttack, self.iconAutoAttack = autoAttack.name, autoAttack.icon;
	local shootWand = C_Spell.GetSpellInfo(5019);
	self.strShootWand, self.iconShootWand = shootWand.name, shootWand.icon;
